...
SELECT DISTINCT ?author ?authorLabel ?date_of_death ?viaf WHERE {
  ?author wdt:P31 wd:Q5;                 # Instance of human
          wdt:P27 wd:Q811;               # Country: Venezuela
          wdt:P570 ?date_of_death;       # Date of death
          wdt:P214 ?viaf                 # VIAF code
  FILTER(?date_of_death < "1956-01-01"^^xsd:dateTime)
...
try:
    ...
    with open("output/nicaraguan_authors.json", 'w', encoding='utf-8') as json_file:
        json.dump(data, json_file, indent=2, ensure_ascii=False)
    
    print(f"✓ Results saved to 'output/nicaraguan_authors.json'")
